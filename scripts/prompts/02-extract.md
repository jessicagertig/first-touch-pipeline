...
- Exclude marketing slogans and taglines outright — do NOT surface a company's polished marketing phrasing as a candidate at all ("data, not code"; "ship without a deploy"; "real-time pass-through"). A slogan can only be repeated, not reacted to, so it just gets parroted back. Surface the concrete thing BEHIND the slogan instead: the case study, the real number, the actual mechanism — material a person can react to in their own plain words (a result, a customer story, a quirky or blunt line they wrote, a specific thing they built).
